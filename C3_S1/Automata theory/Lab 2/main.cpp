...
int get_id(map<string, LiteralT> &table, char *buffer) {
    int result = distance(table.begin(), table.find(buffer));

    if (result >= distance(table.begin(), table.end()))
        result = -1;
    return result;
}
...
void print_output(map<string, LiteralT> &table) {
    ...
    int j = 0;
    string output;
    ifstream fin(R"(C:\git-clone\University\C3_S1\Automata theory\Lab 2\data.txt)");
